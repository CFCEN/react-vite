...
const buildMenuItems = (
  ...
  return sortRoutes(routes)
    ...
    .map<AntdMenuItems[number]>((route, index) => {
      ...
      const children = route.children?.length ? buildMenuItems(route.children, key) : undefined
...
const findActiveMenuState = (
  ...
  sortRoutes(routes).forEach((route, index) => {
    ...
    if (route.children?.length) {
      const childMatch = findActiveMenuState(route.children, pathname, key, [...ancestorKeys, key])
    ...
    if (route.path && isPathMatch(route.path, pathname) && route.path.length > bestMatch.matchedPathLength) {
      bestMatch = {
        selectedKey: key,
